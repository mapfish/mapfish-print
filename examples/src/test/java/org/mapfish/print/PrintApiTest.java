...
public class PrintApiTest extends AbstractApiTest {
  ...
  public void testExampleRequest_NoApp() throws Exception {
    ClientHttpRequest request =
        getPrintRequest(MapPrinterServlet.EXAMPLE_REQUEST_URL, HttpMethod.GET);
    try (ClientHttpResponse response = request.execute()) {
      assertEquals(HttpStatus.OK, response.getStatusCode());
      assertEquals(getJsonMediaType(), response.getHeaders().getContentType());
  ...
  private void testCreateReport(String requestPath, String printSpec) throws Exception {
    ...
    if (hasAppId) {
      ...
      request =
          getPrintRequest(
              appId + MapPrinterServlet.STATUS_URL + "/" + ref + ".json", HttpMethod.GET);
      try (ClientHttpResponse response = request.execute()) {
        assertEquals(HttpStatus.OK, response.getStatusCode());
        assertEquals(getJsonMediaType(), response.getHeaders().getContentType());
    ...
    if (hasAppId) {
      ...
      request =
          getPrintRequest("/" + appId + MapPrinterServlet.REPORT_URL + "/" + ref, HttpMethod.GET);
      try (ClientHttpResponse response = request.execute()) {
        assertEquals(HttpStatus.OK, response.getStatusCode());
        assertEquals(new MediaType("application", "pdf"), response.getHeaders().getContentType());
        assertTrue(response.getBody().read() >= 0);
  ...
  private String getDefaultAppDefaultRequestSample()
  ...
  public void testSecuredTemplate_CreateMap() throws Exception {
    ClientHttpRequest request =
        getPrintRequest(
            "secured_templates" + MapPrinterServlet.CREATE_AND_GET_URL + ".pdf", HttpMethod.POST);
    final String printSpec =
        getPrintSpec("examples/secured_templates/requestData.json")
  ...
  private void execCreateRequestWithAuth(
      String credentials, HttpStatus expectedStatus, String printSpec)
  ...
  private void waitUntilDoneOrError(String ref) {
    ...
    do {
      try {
        ClientHttpRequest request =
            getPrintRequest(MapPrinterServlet.STATUS_URL + "/" + ref + ".json", HttpMethod.GET);
        try (ClientHttpResponse response = request.execute()) {
          final JSONObject statusResult = new JSONObject(getBodyAsText(response));
