...
public class ConfigTest extends PrintTestCase {
    ...
    public void testBestScale() {
        ...
        try {
            ...
            assertEquals("Too small scale => pick the smallest available", 25000.0, config.getBestScale(1), 0.0000001);
            assertEquals("Exact match", 25000, config.getBestScale(25000.0), 0.0000001);
            assertEquals("Just too big => should still take the previous one", 25000, config.getBestScale(25000.1), 0.0000001);
            assertEquals("Normal behaviour", 200000, config.getBestScale(150000), 0.0000001);
            assertEquals("Just a litle before", 200000, config.getBestScale(199999.9), 0.0000001);
