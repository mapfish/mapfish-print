...
public class URLSLDParserPluginTest extends AbstractMapfishSpringTest {
  ...
  @Autowired private SLDParserPlugin parserPlugin;
  ...
  public void testParseStyle() throws Throwable {
    ...
    final Optional<Style> styleOptional =
        parserPlugin.parseStyle(
            ...
            "http://"
                + host
                + "/org/mapfish/print/processor/map/center_wmts_fixedscale/thinline.sld");

    assertTrue(styleOptional.isPresent());

    final Optional<Style> styleOptional2 =
        parserPlugin.parseStyle(configuration, requestFactory, "file://thinline.sld");

    assertTrue(styleOptional2.isPresent());

    final Optional<Style> styleOptional3 =
        parserPlugin.parseStyle(configuration, requestFactory, "file://config.yaml");
