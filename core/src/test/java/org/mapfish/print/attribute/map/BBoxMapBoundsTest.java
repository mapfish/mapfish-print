...
public class BBoxMapBoundsTest {
  ...
  public void testAdjustToScale() {
    ...
    final CenterScaleMapBounds mapBounds =
        new CenterScaleMapBounds(CH1903, 50000, 50000, scaleDenominator);
    ...
    final MapBounds newMapBounds =
        linear.adjustBoundsToNearestScale(
            zoomLevels, 0.05, ZoomLevelSnapStrategy.CLOSEST_LOWER_SCALE_ON_TIE, false, screen, dpi);
    ...
    CenterScaleMapBounds expectedMapBounds =
        new CenterScaleMapBounds(
            CH1903, originalBBox.centre().x, originalBBox.centre().y, expectedScale);
  ...
  public void testAdjustToScaleLatLong() {
    ...
    final CenterScaleMapBounds mapBounds =
        new CenterScaleMapBounds(SPHERICAL_MERCATOR, 5, 5, scaleDenominator);
    final ReferencedEnvelope originalBBox = mapBounds.toReferencedEnvelope(screen);
    ...
    CenterScaleMapBounds expectedMapBounds =
        new CenterScaleMapBounds(
            SPHERICAL_MERCATOR, originalBBox.centre().x, originalBBox.centre().y, expectedScale);
  ...
  public void testAdjustToGeodeticScale() {
    ...
    final CenterScaleMapBounds mapBounds =
        new CenterScaleMapBounds(SPHERICAL_MERCATOR, 400000, 5000000, scaleDenominator);
    final ReferencedEnvelope originalBBox = mapBounds.toReferencedEnvelope(screen);
    ...
    final MapBounds newMapBounds =
        linear.adjustBoundsToNearestScale(
            zoomLevels, 0.005, ZoomLevelSnapStrategy.CLOSEST_LOWER_SCALE_ON_TIE, true, screen, dpi);
    ReferencedEnvelope newBBox = newMapBounds.toReferencedEnvelope(screen);
  ...
  public void testExpandHorizontalFeatureInVerticalMap() {
    ...
    assertEquals(
        2.0,
        expandedBounds.toReferencedEnvelope(null).getWidth()
            / expandedBounds.toReferencedEnvelope(null).getHeight(),
        0.01);

    assertEquals(
        bounds.toReferencedEnvelope(null).getWidth() * 2.0,
        expandedBounds.toReferencedEnvelope(null).getWidth(),
        0.01);

    assertEquals(
        bounds.toReferencedEnvelope(null).getHeight() * 2.0,
        expandedBounds.toReferencedEnvelope(null).getHeight(),
        0.01);
    ...
    assertEquals(
        expandedBounds.toReferencedEnvelope(paintArea).getWidth(),
        adjusted.toReferencedEnvelope(paintArea).getWidth(),
        0.01);
  ...
  public void testExpandVerticalFeatureInHorizontalMap() {
    ...
    assertEquals(
        expandedBounds.toReferencedEnvelope(paintArea).getHeight(),
        adjusted.toReferencedEnvelope(paintArea).getHeight(),
        0.01);
