...
public class GenericMapAttributeTest {
  ...
  public void testPrintClientConfig() {
    ...
    assertEquals(
        "[72,92.2,128,200.5]",
        suggestions
            .get(GenericMapAttribute.JSON_DPI_SUGGESTIONS)
            .toString()
            .replaceAll("\\s+", ""));
    assertEquals(
        "[4000,3000,2000,1000]",
        suggestions
            .get(GenericMapAttribute.JSON_ZOOM_LEVEL_SUGGESTIONS)
            .toString()
            .replaceAll("\\s+", ""));
  ...
  public void testPrintClientConfigWithDefaults() {
    ...
    JSONObject embeddedDefault =
        required.getJSONObject("embedded").getJSONObject(JSON_ATTRIBUTE_DEFAULT);
