 ...
 * <p>
 *     Version 1 is compatible with mapfish print &lt;= v2 and is based on the OpenLayers v2 styling.
 ...
 * <p>
 *     Version 2 uses the same property names as version 1 but has a different structure.
 *     The layout is as follows:
 * </p>
 * <pre><code>
 * {
 ...
 *         <p>
 *             In order to reduce duplication and keep the style definitions small, default values can be
 *             specified.
 *             The default values in the root (style level) will be used in all symbolizers if
 *             the value is not defined.
 *             The style level default will apply to all symbolizers defined in the system.
 *         </p>
 *         <p>
 *             The only difference between a value and a default is that the default has a well known name,
 *             therefore defaults can also be used as values.
 *         </p>
 *     </li>
 *     <li>
 *         All the styling rules (At least one is required)
 *         <p>
 *             A styling rule has a key which is the filter which selects the features that the rule will
 *             be used to draw and the rule definition object.
 *         </p>
 ...
 *             However it is ignored by geotools and the distance is always in the crs of the geometry
 *             projection.
 *         </p>
 ...
 *                 <p>
 *                         Each rule can also have defaults.  If the style and the rule have a default for
 *                         the same property the rule will override the style default.  All defaults can be
 *                         (of course) overridden by a value in a symbolizer.
 *                     </p>
 *                 </li>
 *                 <li>
 *                     minScale (optional)
 ...
 *                     <p>
 *                         A symbolizer must have a type property (point, line, polygon, text) which
 *                         indicates the type of symbolizer and it has the attributes for that type of
 *                         symbolizer.  All values have defaults so it is possible to define a symbolizer
 *                         as with only the type property. The only exception is that the "text" symbolizer
 *                         needs a label property.
 *                     </p>
 *                 </li>
 *             </ul>
 *     </li>
 ...
 * The items in the list below are the properties that can be set on the different symbolizers.
 * In brackets list the symbolizers the values can apply to.
 * <p>
 *     Most properties can be static values or ECQL expressions.  If the property has <code>[ ]</code>
 ...
 *     you will have to enter: <code>['propertyValue']</code> (where propertyValue
 ...
 *                  shape://coarrow, shape://ccarrow</li>
 ...
 *         end of a line.
 *         <p>
 *             Options:  butt (sharp square edge), round (rounded edge), and square (slightly elongated
 ...
 *     it will be assumed that the geometry is a line and this property defines how far from the center of
 *     the line the label should be drawn.</li>
 * </ul>
 * <p></p>
 ...
 * <p>
 *     The label property defines what label will be drawn for a given feature.  The value is either a
 ...
 * <p>
 *     A halo is a space around the drawn label text that is color (using the halo properties).  A label
 *     with a halo is like the drawn label text with a buffer around the label text drawn using the halo
 *     properties.  This allows the label to be clearly visible regardless of the background.  For example
 *     if the text is black and the halo is with, then the text will always be readable thanks to the white
 *     buffer around the label text.
 * </p>
 *
 * <h3 id="otherproperties">Font/weight/style/color/opacity
 * <a class="headerlink" href="#otherproperties">¶</a></h3>
 * <p>
 *     The Font/weight/style/color/opacity properties define how the label text is drawn.  They are for the
 *     most part equivalent to the similarly named css and SLD properties.
 * </p>
 *
 ...
 *     placement and <em>only one</em> type of placement can be used. The type of placement is determined
 *     by inspecting the properties in the text symbolizer and if the <em>labelPerpendicularOffset</em>
 *     property is defined then a line placement will be created for the text symbolizer.
 * </p>
 * <p>
 *     It is important to realize that since only one type of placement can be used, an error will be
 ...
 * <p>
 *     Point placement defines an <em>anchor point</em> which is the point to draw the text relative to.
 *     For example an anchor point of 0.5, 0.5 ("labelAnchorPointX" : "0.5", "labelAnchorPointY" : "0.5")
 *     would position the start of the label at the center of the geometry.
 * </p>
 * <p>
 ...
 * <p>
 *     There are two ways to define the anchor point, either the <em>labelAnchorPointX/Y</em> properties
 *     are set or the <em>labelAlign</em> property is set.  If both are defined then the
 ...
 *      two labels will be allowed to overlap. Symbolizers with conflict resolution off are considered
 *      outside of the conflict resolution game, they don’t reserve area and can overlap with other labels.
 *  </li>
