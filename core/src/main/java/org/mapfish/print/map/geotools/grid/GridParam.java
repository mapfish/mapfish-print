...
public final class GridParam extends AbstractLayerParams {
   ...
   *
   * <p>Either {@link #spacing} or {@link #numberOfLines}
   *
   * <p>If spacing is defined then {@link #origin} must also be defined
   */
  @OneOf("spacing")
  @Requires("origin")
   ...
   *
   * <p>This is required if {@link #spacing} is defined.
   */
  @HasDefaultValue public double[] origin;

  /**
   * The x,y number of grid lines.
   *
   * <p>The x is the number of lines that run vertically along the page.
   */
  @OneOf("spacing")
   ...
   * the type is point or line and will respect {@link #gridColor} and {@link #haloColor} and {@link
   * #haloRadius}. If {@link #gridType} is {@link GridType#POINTS} then the style will be crosses
   * with a haloRadius sized halo around the cross. If {@link GridType#LINES} then the style will be
   ...
   */
  @HasDefaultValue public Boolean renderAsSvg = false;

  /**
   * The number of points that will be in the grid line (if the gridType is LINES). If the line will
   * be curved (for certain projections) then the more points the smoother the curve.
   *
   * <p>The default number of points is {@value DEFAULT_POINTS_IN_GRID_LINE}.
   */
  @HasDefaultValue public int pointsInLine = DEFAULT_POINTS_IN_GRID_LINE;
   ...
   * to use this parameter. For the format syntax, see <a
   * href="https://docs.oracle.com/javase/7/docs/api/java/util/Formatter.html">java.util.Formatter</a>.
   */
   ...
   * Defaults to 0.
   */
  @HasDefaultValue public double horizontalYOffset = 0;
