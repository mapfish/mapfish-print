...
public class WmsLayerParam extends AbstractWMXLayerParams {
  ...
  /**
   * The styles to apply to the layers. If this is defined there should be the same number as the
   * layers and the style are applied to the layer in the {@link #layers} field.
   */
  @HasDefaultValue public String[] styles;

  /** The WMS version to use when making requests. */
  @HasDefaultValue public String version = "1.1.1";

  /** If true transform the map angle to customParams.angle for GeoServer, and MapServer. */
  @HasDefaultValue public boolean useNativeAngle = true;
   ...
   */
  @HasDefaultValue public ServerType serverType;
   ...
   * long. Tested only with GeoServer.
   */
  @HasDefaultValue public HttpMethod method = HttpMethod.GET;
  ...
  public void postConstruct() throws URISyntaxException {
    ...
    // OpenLayers 2 compatibility.  It will post a single empty style no matter how many layers
    // there are
      ...
      Assert.isTrue(
          ...
          String.format(
              "If styles are defined then there must be one for each layer.  Number of"
                  + " layers: %s\nStyles: %s",
              this.layers.length, Arrays.toString(this.styles)));
    }
