...
/**
 * Strategy object for rendering XYZ based layers.
 *
 * @author DidierCoignet.
 */
public final class XyzLayer extends AbstractTiledLayer {

    private final XyzLayerParam params;
     ...
     */
    public XyzLayer(final ForkJoinPool forkJoinPool,
                    final StyleSupplier<GridCoverage2D> styleSupplier,
                    final XyzLayerParam param) {
    ...
    protected TileCacheInformation createTileInformation(final MapBounds bounds, final Rectangle paintArea, final double dpi,
        ...
        return new XyzTileCacheInformation(bounds, paintArea, dpi, isFirstLayer);
    }

    private final class XyzTileCacheInformation extends TileCacheInformation {

        private final Scale scale;
        private final int resolutionIndex;

        public XyzTileCacheInformation(final MapBounds bounds, final Rectangle paintArea, final double dpi,
                                       final boolean isFirstLayer) {
            super(bounds, paintArea, dpi, XyzLayer.this.params);

            final double targetResolution = bounds.getScaleDenominator(paintArea, dpi).toResolution(bounds.getProjection(), dpi);

            Double[] resolutions = XyzLayer.this.params.resolutions;
            int pos = resolutions.length - 1;
            double result = resolutions[pos];
            for (int i = resolutions.length - 1; i >= 0; --i) {
                double cur = resolutions[i];
                if (cur <= targetResolution * XyzLayer.this.params.resolutionTolerance) {
                    result = cur;
                    pos = i;
                }
            }
        ...
        public ClientHttpRequest getTileRequest(final MfClientHttpRequestFactory httpRequestFactory,
            ...
            if (commonUrl.contains("{x}") && commonUrl.contains("{z}")
                ...
                path.append('.').append(XyzLayer.this.params.imageExtension);

                uri = new URI(commonUrl + path.toString());
            }

            return httpRequestFactory.createRequest(
                    URIUtils.addParams(uri, XyzLayerParam.convertToMultiMap(XyzLayer.this.params.customParams),
        ...
        public Double getLayerDpi() {
            return XyzLayer.this.params.dpi;
        }

        @Override
        public Dimension getTileSize() {
            return XyzLayer.this.params.getTileSize();
        }

        @Nonnull
        @Override
        protected ReferencedEnvelope getTileCacheBounds() {
            return new ReferencedEnvelope(XyzLayer.this.params.getMaxExtent(), this.bounds.getProjection());
        }
    }
}
