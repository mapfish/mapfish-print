 ...
 *
 * <p>The objects are created using spring dependency injection so that the methods are correctly
 * wired using spring.
 *
 * <p>If an object has the interface HashConfiguration then this class will inject the configuration
...
public final class MapfishPrintConstructor extends Constructor {
  ...
  private static final org.slf4j.Logger LOGGER =
      LoggerFactory.getLogger(MapfishPrintConstructor.class);
  ...
  public MapfishPrintConstructor(final ConfigurableApplicationContext context) {
    ...
    Map<String, ConfigurationObject> yamlObjects =
        context.getBeansOfType(ConfigurationObject.class);
    for (Map.Entry<String, ConfigurationObject> entry : yamlObjects.entrySet()) {
      final BeanDefinition beanDefinition =
          context.getBeanFactory().getBeanDefinition(entry.getKey());
