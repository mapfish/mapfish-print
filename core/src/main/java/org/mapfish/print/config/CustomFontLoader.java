 ...
 *
 * <p>To add a custom font, the file `mapfish-spring-custom-fonts.xml` must be overridden.
 *
 * <p>Warning: the fonts added using this are not available to Jasper Reports. They can only be used
 * in layer styles.
 */
  ...
  private void registerJasperFonts() {
    ...
    for (FontFamily family : families) {
      for (int style = 0; style <= 3; ++style) {
        ...
        final Font font =
            FontUtil.getInstance(DefaultJasperReportsContext.getInstance())
