...
public class RegistryJobQueue implements JobQueue {
  ...
  public final long getAverageTimeSpentPrinting() {
    return this.registry.opt(TOTAL_PRINT_TIME, 0L)
        / this.registry.opt(NB_PRINT_DONE, 1).longValue();
  }
  ...
  private void store(final PrintJobStatus printJobStatus) throws JSONException {
    ...
    metadata.put(
        JSON_ACCESS_ASSERTION, this.assertionPersister.marshal(printJobStatus.getAccess()));
  ...
  private PrintJobStatusImpl load(final String referenceId)
