...
public class ScalebarAttribute
    extends ReflectiveAttribute<ScalebarAttribute.ScalebarAttributeValues> {
  ...
  public final ScalebarAttributeValues createValue(final Template template) {
    return new ScalebarAttributeValues(
        new Dimension(this.width, this.height), this.createSubReport);
  }
  ...
  /**
   * The height of the scalebar in pixels. This value should match the height of the sub-report in
   * the JasperReport template.
   *
   * @param height Height
   */
  ...
  public class ScalebarAttributeValues {
     ...
     *
     * <ul>
     *   <li>"line": A simple line with graduations.
     *   <li>"bar" (default): A thick bar with alternating black and white zones marking the
     *       intervals. The colors can be customized by changing the properties `color` and
     *       `barBgColor`.
     *   <li>"bar_sub": Like "bar", but with little ticks for the labels.
     * </ul>
     */
    @HasDefaultValue public String type = Type.BAR.getLabel();

    /**
     * The unit to use.
     *
     * <p>The unit can be any of:
     *
     * <ul>
     *   <li>m (mm, cm, m or km)
     *   <li>ft (in, ft, yd, mi)
     ...
     * </ul>
     *
     * <p>If the value is too big or too small, the module will switch to one of the unit in
     * parenthesis (the same unit is used for every interval). If this behavior is not desired, the
     * `lockUnits` parameter will force the declared unit (or map unit if no unit is declared) to be
     * used for the scalebar.
     */
    @HasDefaultValue public String unit = null;

    /** Use geodetic measurement calculations for the scalebar. */
    @HasDefaultValue public boolean geodetic = false;

    /**
     * Force that the given unit is used (default: false). For example if the unit is set to meters
     * and `lockUnits` is enabled, then meters is always used, even when kilometers would create
     * nicer values.
     */
    @HasDefaultValue public Boolean lockUnits = false;

    /** The number of intervals (default: 3). There must be at least two intervals. */
    @HasDefaultValue public Integer intervals = DEFAULT_INTERVALS;

    /**
     * Should sub-intervals be shown? Default: false
     *
     * <p>The main intervals are divided into additional sub-intervals to provide visual guidance.
     * The number of sub-intervals depends on the length of an interval.
     */
    @HasDefaultValue public Boolean subIntervals = false;

    /** The thickness of the bar or the height of the tick marks on the line (in pixel). */
    @HasDefaultValue public Integer barSize = null;

    /** The thickness of the lines or the bar border (in pixel). */
    @HasDefaultValue public Integer lineWidth = null;

    /** The distance between scalebar and labels (in pixel). */
    @HasDefaultValue public Integer labelDistance = null;

    /** The padding around the scalebar (in pixel). */
    @HasDefaultValue public Integer padding = null;

    /** The font used for the labels (default: Helvetica). */
    @HasDefaultValue public String font = DEFAULT_FONT;

    /** The font size (in pt) of the labels (default: 12). */
    @HasDefaultValue public Integer fontSize = DEFAULT_FONT_SIZE;

    /** The font color of the labels (default: black). */
    @HasDefaultValue public String fontColor = DEFAULT_FONT_COLOR;

    /** The color used to draw the bar and lines (default: black). */
    @HasDefaultValue public String color = DEFAULT_COLOR;

    /**
     * The color used to draw the alternating blocks for style "bar" and "bar_sub" (default: white).
     */
    @HasDefaultValue public String barBgColor = DEFAULT_BAR_BG_COLOR;

    /** The background color for the scalebar graphic (default: rgba(255, 255, 255, 0)). */
    @HasDefaultValue public String backgroundColor = DEFAULT_BACKGROUND_COLOR;
     ...
     *
     * <ul>
     *   <li>"horizontalLabelsBelow" (default): Horizontal scalebar and the labels are shown below
     *       the bar.
     *   <li>"horizontalLabelsAbove": Horizontal scalebar and the labels are shown above the bar.
     *   <li>"verticalLabelsLeft": Vertical scalebar and the labels are shown left of the bar.
     *   <li>"verticalLabelsRight": Vertical scalebar and the labels are shown right of the bar.
     * </ul>
     */
    @HasDefaultValue public String orientation = Orientation.HORIZONTAL_LABELS_BELOW.getLabel();

    /** Rotate the label to some degree. */
    @HasDefaultValue public float labelRotation = 0.0f;

    /** The horizontal alignment of the scalebar inside the scalebar graphic (default: left). */
    @HasDefaultValue public String align = HorizontalAlign.LEFT.getLabel();

    /** The vertical alignment of the scalebar inside the scalebar graphic (default: bottom). */
    @HasDefaultValue public String verticalAlign = VerticalAlign.BOTTOM.getLabel();
     ...
     */
    @HasDefaultValue public Boolean renderAsSvg = true;
