...
public abstract class GenericMapAttribute
   ...
   *
   * <p>If a request is made with a higher DPI, the request fails.
   *
  ...
  /**
   * Suggestions for DPI values to use. Typically these are used by the client to create a UI for a
   * user.
   *
  ...
  /**
   * The width of the map in pixels. This value should match the width of the sub-report in the
   ...
   *
   * <ul>
   *   <li>CLOSEST_LOWER_SCALE_ON_TIE: Find the closest zoom level. If the targetScale is directly
   ...
   *   <li>HIGHER_SCALE: Always choose the zoom-level that is just higher than the target value.
   *   <li>LOWER_SCALE: Always choose the zoom-level that is just lower than the target value.
   * </ul>
   *
   * <p>
   *
    ...
    if (this.getMaxDpi() != null && this.getDpiSuggestions() != null) {
      for (double dpi : this.getDpiSuggestions()) {
        if (dpi < 1 || dpi > this.getMaxDpi()) {
          validationErrors.add(
              new ConfigurationException(
                  "dpiSuggestions contains an invalid value: "
                      + dpi
                      + " in "
                      + getClass().getName()));
        }
  ...
  public abstract class GenericMapAttributeValues {
     ...
     *
     * <p>If this property is set to `true`, the style definitions will be scaled to the target DPI
     * value.
     */
    @HasDefaultValue public boolean dpiSensitiveStyle = true;

    private List<MapLayer> mapLayers;

    /**
     * Whether to merge all layers into one. This is great to reduce pdf size but needs layers
     * having an imagebufferscaling of 1. Also see the pdfA option of the print config.
     */
     ...
     */
    protected GenericMapAttributeValues(final Template template) {
     ...
     */
    protected GenericMapAttributeValues(
    ...
    /**
     * Validate the values provided by the request data and construct MapBounds and parse the
     * layers.
     */
    ...
    /**
     * Return the DPI value for the map. This method is abstract because the dpi value is optional
     * for the overview map, but must be given for the normal map. So, in the overview map the field
     * is defined with a @HasDefaultValue annotation.
     */
    ...
    /**
     * Set the JSON layer definition. This method is abstract for the same reasons as {@link
     * #getDpi()}.
     *
    ...
    /**
     * Return true if requestData has useNearestScale and configuration has some zoom levels
     * defined.
     */
