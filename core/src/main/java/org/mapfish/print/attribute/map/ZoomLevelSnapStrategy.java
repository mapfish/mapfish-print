...
public enum ZoomLevelSnapStrategy {
  ...
  CLOSEST_LOWER_SCALE_ON_TIE {
    ...
    protected SearchResult search(
        final Scale scale, final double tolerance, final ZoomLevels zoomLevels) {
  ...
  /**
   * Search the provided zoomLevels for the scale that is the closest according to the current
   * strategy.
   *
   * @param scale the reference scale
   * @param tolerance the amount from one of the zoomLevels to still be considered <em>at</em> the
   *     scale. This is important for all strategies other than CLOSEST in order to prevent the
   *     scale from jumping to a different version even when it is very close to one of the
   *     zoomLevels.
