...
public class ZoomToFeatures {
   ...
   *
   * <ul>
   *   <li><code>extent</code> (default): Set the extent of the map so that all features are
   *       visible.
   *   <li><code>center</code>: Set the center of the map to the center of the extent of the
   *       features.
   * </ul>
   */
  @HasDefaultValue public ZoomType zoomType = ZoomType.EXTENT;

  /**
   * The name of the layer whose features will be used. If not set, the features of all vector
   * layers will be used.
   */
  @HasDefaultValue public String layer;

  /** The minimum scale that the map is zoomed to. */
  @HasDefaultValue public Double minScale;

  /** The minimum margin (in px) from the features to the map border (default: 10). */
  @HasDefaultValue public Integer minMargin = DEFAULT_ZOOM_TO_MIN_MARGIN;
