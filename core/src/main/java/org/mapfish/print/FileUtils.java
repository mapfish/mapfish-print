...
public final class FileUtils {
  ...
  /**
   * Check if the url is a file url which is either relative to the configuration directory or
   * refers to a file in the configuration directory.
   *
   * <p>The correct file url is returned or the original if not a file url. If an illegal file url
   * (one that does not refer to a file within the configuration directory or is relative to the
   * configuration directory) then an exception is thrown.
   *
  ...
  public static URL testForLegalFileUrl(final Configuration configuration, final URL url) {
    ...
    if (protocol.equalsIgnoreCase("file")) {
      try {
        ...
        File file =
            new File(
                configuration.getDirectory(), url.toExternalForm().substring("file://".length()));
  ...
  /**
   * Verify that the file is within the base directory. {@link
   * org.mapfish.print.IllegalFileAccessException} will be thrown if the assertion does not hold.
   *
