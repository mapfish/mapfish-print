 ...
 *
 * <p>See also: <a href="attributes.html#!scalebar">!scalebar</a> attribute
...
public class CreateScalebarProcessor
  ...
  public final Output execute(final Input values, final ExecutionContext context) throws Exception {
    ...
    if (values.scalebar.isCreateSubReport()) {
      final URI scalebarSubReport =
          createScalebarSubReport(
  ...
  private URI createScalebarGraphic(final Input values)
      throws IOException, ParserConfigurationException {
    final ScalebarGraphic scalebar = new ScalebarGraphic();
    return scalebar.render(
        values.mapContext, values.scalebar, values.tempTaskDirectory, values.template);
  }

  private URI createScalebarSubReport(
      final File printDirectory, final Dimension size, final List<URI> graphics, final double dpi)
