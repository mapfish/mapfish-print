...
public final class ProcessorExecutionContext {
   ...
   *
   * <p>In case the processor is already running, has already finished or if not all requirements
   * have finished, the processor can not start. If the above conditions are fulfilled, the
   * processor is added to the list of running processors, and is expected to be started from the
   * caller.
   *
  ...
  public boolean tryStart(final ProcessorGraphNode processorGraphNode) {
    ...
    try {
      if (isRunning(processorGraphNode)
   ...
   *
   * <p>All processors are supposed to check this flag frequently and terminate the execution if
   * requested.
   */
  public void cancel() {
    this.context.cancel();
