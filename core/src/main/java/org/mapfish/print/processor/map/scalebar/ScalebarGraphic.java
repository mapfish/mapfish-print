...
public class ScalebarGraphic {
  ...
  private static void tryLayout(
    ...
    if (tryNumber > MAX_NUMBER_LAYOUTING_TRIES) {
      ...
      LOGGER.error(
          "layouting the scalebar failed (unit: {}, scale: {})", scaleUnit, scaleDenominator);
      return;
    }
    ...
    final float intervalLengthInPixels =
        (float) scaleUnit.convertTo(intervalLengthInWorldUnits / scaleDenominator, DistanceUnit.PX);
    ...
    if (fitsAvailableSpace(
      ...
      double nextIntervalDistance =
          getNearestNiceValue(
              intervalLengthInWorldUnits * 0.9, scaleUnit, scalebarParams.lockUnits);
      tryLayout(
          graphics2D, scaleUnit, scaleDenominator, nextIntervalDistance, settings, tryNumber + 1);
    }
  }
  ...
  private static boolean fitsAvailableSpace(
    ...
    if (scalebarParams.getOrientation().isHorizontal()) {
      return scalebarParams.intervals * intervalWidthInPixels
              + leftLabelMargin
              + rightLabelMargin
              + 2 * settings.getPadding()
   ...
   *
   * <p>Creates the drawer which draws the scalebar.
   */
  ...
  /**
   * Reduce the given value to the nearest smaller 1 significant digit number starting with 1, 2 or
   * 5.
   *
  ...
  public final URI render(
    ...
    final double scaleDenominator =
        scale.getDenominator(
            scalebarParams.geodetic, bounds.getProjection(), dpi, bounds.getCenter());
    ...
    final double maxLengthInPixel =
        (scalebarParams.getOrientation().isHorizontal())
    ...
    if (template.getConfiguration().renderAsSvg(scalebarParams.renderAsSvg)) {
      ...
      final SVGGraphics2D graphics2D =
          CreateMapProcessor.createSvgGraphics(scalebarParams.getSize());

      try {
        tryLayout(
            graphics2D, scaleUnit, scaleDenominator, niceIntervalLengthInWorldUnits, settings, 0);
