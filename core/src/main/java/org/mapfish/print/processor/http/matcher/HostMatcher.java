...
public abstract class HostMatcher extends AbstractMatcher {
  ...
  /**
   * A regex that will be ran against the host name. If there is a match then the matcher accepts
   * the uri.
   */
  ...
  /**
   * If the subclass has its own checks or if it has a different validation method this method can
   ...
   * /.
   *
   * <p>The regular expression used are the ones supported by java: <a
