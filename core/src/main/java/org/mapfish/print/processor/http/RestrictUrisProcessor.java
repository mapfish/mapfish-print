 ...
 * or rejected.
 *
 * <p>Usage of processor is as follows:
 *
 * <pre><code>
 * - !restrictUris
 *   matchers:
 ...
 * </code></pre>
 *
 * <p>By default a matcher allows the URL, but it can be setup to reject the URL (by setting reject
 * to true). The first matcher that matches will be the one picking the final outcome. If no matcher
 * matches, the URI is rejected. So, for example, you can allow every URLs apart from the internal
 * URLs like that:
 *
 * <pre><code>
 * - !restrictUris
 ...
 * </code></pre>
 *
 * <p>If the Print service is in your DMZ and needs to allow access to any WMS server, it is
 * strongly recommended to have a configuration like the previous one in order to avoid having the
 * Print service being used as a proxy to access your internal servers.
 *
 * <p><strong>Note:</strong> if this class is part of a CompositeClientHttpRequestFactoryProcessor
 * (!configureHttpRequests) then it should be the last one so that the checks are done after all
 * changes to the URIs [[examples=http_processors]]
 *
