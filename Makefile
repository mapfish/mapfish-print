...
acceptance-tests-run: .env
	...
	docker compose $(DOCKER_COMPOSE_ARGS) exec -T print sh -c 'for i in $(shell seq 1 24); do curl --fail http://geoserver:8080/geoserver/www/map-data/test-data/simple-squares-geojson.json && exit 0; sleep 5; done; exit 1'
	docker compose $(DOCKER_COMPOSE_ARGS) exec -T print curl --fail http://geoserver:8080/geoserver/www/map-data/test-data/simple-squares-geojson.json
